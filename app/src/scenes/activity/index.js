...
const Activities = ({ date, user, project }) => {
  ...
  async function onDelete(i) {
    if (window.confirm("Are you sure?")) {
      const activityToDelete = activities[i];
      try {
        await api.remove(`/activity/${activityToDelete._id}`);
        const updatedActivities = activities.filter(activity => activity._id !== activityToDelete._id);
        setActivities(updatedActivities);
        toast.success(`Deleted ${activityToDelete.project}`);
      } catch (error) {
